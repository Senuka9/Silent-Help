...
/**
 * Anxious recovery path orchestrator at /recovery/anxious.
 *
 * Renders the current step, persists progress to localStorage so the user
 * can resume mid-flow, and uses the emotion theme for an "anxious" accent.
 * Bypasses AppLayout via the parent /recovery/layout.tsx (auth-gated only).
 */
...
import { StepBraveStep } from '@/components/recovery/anxious/StepBraveStep';
import { StepDone } from '@/components/recovery/anxious/StepDone';
import { createJournalEntry } from '@/lib/api';
import { toast } from 'sonner';

const STEP_ORDER: AnxiousStep[] = ['ground', 'reframe', 'worry', 'brave', 'done'];

export default function AnxiousRecoveryPage() {
    const router = useRouter();
    const { profile, loadProfile } = useWellness();
    const [session, setSession] = useState<AnxiousSession | null>(null);
    const [showResumePrompt, setShowResumePrompt] = useState(false);
    const [completions, setCompletions] = useState(0);

    const theme = resolveEmotion('anxious');
    const stress = resolveStressLevel(profile?.stressLevel);

    // Initial load: profile + any paused session.
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (existing && existing.currentStep !== 'ground' && existing.currentStep !== 'done') {
            setSession(existing);
            setShowResumePrompt(true);
        } else {
            setSession(emptySession(stress.tier));
        }
        // We only want this to run once on mount, after `stress` is initially resolved.
    ...
    const onJournal = useCallback(async () => {
        if (!session) return;
        const parts: string[] = [];
        parts.push(`# Anxious recovery session\n`);
        if (session.sensoryItems.length > 0) {
            parts.push(`**Grounded with:**`);
            parts.push(
                session.sensoryItems
                    .map((i) => `- ${i.sense}: ${i.value}`)
                    .join('\n'),
            );
        }
        if (session.reframe.thought) {
            parts.push(`\n**The anxious thought:** "${session.reframe.thought}"`);
            if (session.reframe.fear) parts.push(`**Fear underneath:** ${session.reframe.fear}`);
            if (session.reframe.reality) parts.push(`**Reframe:** ${session.reframe.reality}`);
            if (session.stressTier === 'high' || session.stressTier === 'medium-high') {
                parts.push(
                    `**Probability:** felt ${session.reframe.feltProbability}% → rational ${session.reframe.rationalProbability}%`,
                );
            }
        }
        if (session.worry) {
            parts.push(
                `\n**Worry window:** ${session.worry.worryText} — scheduled for ${session.worry.label}`,
            );
        }
        if (session.brave.tinyStep) {
            parts.push(`\n**Goal:** ${session.brave.goal}`);
            parts.push(`**Tiny brave step:** ${session.brave.tinyStep}`);
        }
        try {
    ...
    return (
        ...
        >
            <Aurora
                intensity="strong"
                colors={[`${theme.accent}aa`, `${theme.accent2}88`, 'rgba(56,189,248,0.35)']}
            />
            ...
            <header className="pointer-events-none fixed inset-x-0 top-0 z-20 p-4">
                <div className="pointer-events-auto mx-auto flex max-w-3xl items-center justify-between gap-4 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 backdrop-blur-lg">
                    <div className="flex items-center gap-2 text-xs uppercase tracking-[0.22em] text-white/60">
                        <span>{theme.icon} Anxious path</span>
                        <span
                                ...
                                onComplete={(res) => {
                                    setSession((s) =>
                                        s
                                            ? {
                                                  ...s,
                                                  breathsCompleted: res.breathsCompleted,
                                                  sensoryItems: res.sensoryItems,
                                                  pmrCompleted: res.pmrCompleted,
                                                  currentStep: 'reframe',
                                              }
                                            : s,
                                    );
                        ...
                        >
                            <StepReframe
                                ...
                                onComplete={(data) => {
                                    setSession((s) =>
                                        s
                                            ? { ...s, reframe: data, currentStep: 'worry' }
                                            : s,
                                    );
                                }}
                            />
                    ...
                    {session.currentStep === 'worry' && (
                        ...
                        >
                            <StepWorryWindow
                                stressTier={session.stressTier}
                                accent={theme.accent}
                                initial={session.worry}
                                onComplete={(w) => {
                                    setSession((s) =>
                                        s ? { ...s, worry: w, currentStep: 'brave' } : s,
                                    );
                                }}
                            />
                        </motion.div>
                    ...
                    {session.currentStep === 'brave' && (
                        ...
                        >
                            <StepBraveStep
                                stressTier={session.stressTier}
                                accent={theme.accent}
                                initial={session.brave}
                                onComplete={(data) => {
                                    setSession((s) => {
                                        if (!s) return s;
                                        const next: AnxiousSession = {
                                            ...s,
                                            brave: data,
                                            currentStep: 'done',
                                        };
                        ...
                        >
                            ...
                            {completions > 0 && (
                                <div className="mb-8 text-center text-xs uppercase tracking-[0.2em] text-white/40">
                                    {completions} anxious path{completions === 1 ? '' : 's'} completed
                                </div>
                            )}
                        </motion.div>
                    )}
